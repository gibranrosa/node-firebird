...
SQLVarText.prototype.decode = function(data) {
    var ret;
    if (this.subType > 1) {
        ret = data.readText(this.length, this.options.encoding || DEFAULT_ENCODING);
    } else {
...
SQLVarString.prototype.decode = function(data) {
    var ret;
    if (this.subType > 1) {
        ret = data.readString(this.options.encoding || DEFAULT_ENCODING)
    } else {
        ret = data.readBuffer()
    }
    if (!data.readInt()) {
...
SQLParamString.prototype.encode = function(data) {
    if (this.value != null) {
        data.addText(this.value, this.options.encoding || DEFAULT_ENCODING);
        data.addInt(0);
    } else {
        data.addInt(1);
    }
...
SQLParamString.prototype.calcBlr = function(blr) {
    blr.addByte(blr_text);
    var len = this.value ? Buffer.byteLength(this.value, this.options.encoding || DEFAULT_ENCODING) : 0;
    blr.addWord(len);
};
...
ServiceManager.prototype._createOutputStream = function (optread, buffersize, callback) {
    ...
    t.__proto__._read = function () {
        ...
        fct.call(self, { buffersize: buffersize }, function (err, data) {
            if (err) {
                selfread.push(err.message, this.options.encoding || DEFAULT_ENCODING);
                return;
            }
            if (data.line && data.line.length)
                selfread.push(data.line, this.options.encoding || DEFAULT_ENCODING);
            else
                selfread.push(null);
        });
    }
...
ServiceManager.prototype.backup = function (options, callback) {
    ...
    blr.addString2(isc_spb_dbname, dbpath, this.options.encoding || DEFAULT_ENCODING);
    for (var i = 0; i < bckfiles.length; i++) {
        blr.addString2(isc_spb_bkp_file, bckfiles[i].filename, this.options.encoding || DEFAULT_ENCODING);
        if (i != bckfiles.length - 1) // not the end, so we need to write the size of this part (gsplit)
            blr.addString2(isc_spb_bkp_length, bckfiles[i].sizefile, this.options.encoding || DEFAULT_ENCODING);
    }
    if (factor)
...
ServiceManager.prototype.nbackup = function (options, callback) {
    ...
    blr.addString2(isc_spb_dbname, dbpath, this.options.encoding || DEFAULT_ENCODING);
    blr.addString2(isc_spb_dbname, bckfile, this.options.encoding || DEFAULT_ENCODING);
    blr.addByteInt32(isc_spb_nbk_level, level);
    blr.addString2(isc_spb_nbk_direct, direct, this.options.encoding || DEFAULT_ENCODING);
    var opts = 0;
...
ServiceManager.prototype.restore = function(options, callback) {
    ...
    for (var i = 0; i < bckfiles.length; i++) {
        blr.addString2(isc_spb_bkp_file, bckfiles[i], this.options.encoding || DEFAULT_ENCODING);
    }
    blr.addString2(isc_spb_dbname, dbfile, this.options.encoding || DEFAULT_ENCODING);
    ...
    if (fixfssdata) blr.addString2(isc_spb_res_fix_fss_data, fixfssdata, this.options.encoding || DEFAULT_ENCODING);
    if (fixfssmetadata) blr.addString2(isc_spb_res_fix_fss_metadata, fixfssmetadata, this.options.encoding || DEFAULT_ENCODING);
    var opts = 0;
...
ServiceManager.prototype.nrestore = function (options, callback) {
    ...
    for (var i = 0; i < bckfiles.length; i++) {
        blr.addString2(isc_spb_nbk_file, bckfiles[i], this.options.encoding || DEFAULT_ENCODING);
    }
    blr.addString2(isc_spb_dbname, dbpath, this.options.encoding || DEFAULT_ENCODING);
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
...
ServiceManager.prototype.getStats = function (options, callback) {
    ...
    if (opts)
        blr.addByteInt32(isc_spb_options, opts);
    if (objects) blr.addString2(isc_spb_command_line, objects, this.options.encoding || DEFAULT_ENCODING);
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
            doError(new Error(err), callback);
...
ServiceManager.prototype.getUsers = function (username, callback) {
    ...
    if (username) blr.addString2(isc_spb_sec_username, username, this.options.encoding || DEFAULT_ENCODING);
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
            doError(new Error(err), callback);
            return;
...
ServiceManager.prototype.addUser = function (username, password, options, callback) {
    ...
    blr.addString2(isc_spb_sec_username, username, this.options.encoding || DEFAULT_ENCODING);
    blr.addString2(isc_spb_sec_password, password, this.options.encoding || DEFAULT_ENCODING);
    if (rolename) blr.addString2(isc_dpb_sql_role_name, rolename, this.options.encoding || DEFAULT_ENCODING);
    if (groupname) blr.addString2(isc_spb_sec_groupname, groupname, this.options.encoding || DEFAULT_ENCODING);
    if (firsname) blr.addString2(isc_spb_sec_firstname, firsname, this.options.encoding || DEFAULT_ENCODING);
    if (middlename) blr.addString2(isc_spb_sec_middlename, middlename, this.options.encoding || DEFAULT_ENCODING);
    if (lastname) blr.addString2(isc_spb_sec_lastname, lastname, this.options.encoding || DEFAULT_ENCODING);
...
ServiceManager.prototype.editUser = function (username, options, callback) {
    ...
    blr.addString2(isc_spb_sec_username, username, this.options.encoding || DEFAULT_ENCODING);
    if (password) blr.addString2(isc_spb_sec_password, password, this.options.encoding || DEFAULT_ENCODING);
    if (rolename) blr.addString2(isc_dpb_sql_role_name, rolename, this.options.encoding || DEFAULT_ENCODING);
    if (groupname) blr.addString2(isc_spb_sec_groupname, groupname, this.options.encoding || DEFAULT_ENCODING);
    if (firsname) blr.addString2(isc_spb_sec_firstname, firsname, this.options.encoding || DEFAULT_ENCODING);
    if (middlename) blr.addString2(isc_spb_sec_middlename, middlename, this.options.encoding || DEFAULT_ENCODING);
...
ServiceManager.prototype.startTrace = function (options, callback) {
    ...
    blr.addString2(isc_spb_trc_cfg, configfile, this.options.encoding || DEFAULT_ENCODING);
    blr.addString2(isc_spb_trc_name, tracename, this.options.encoding || DEFAULT_ENCODING);
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
            doError(new Error(err), callback);
            return;
            ...
            while (true) {
                ...
                switch (op){
                    ...
                    case isc_arg_sql_state:
                        ...
                        if (item.params) {
                            var str = data.readString(this.options.encoding || DEFAULT_ENCODING);
                            item.params.push(str);
                        } else
                            item.params = [data.readString(this.options.encoding || DEFAULT_ENCODING)];
...
Connection.prototype.connect = function (database, callback) {
    ...
    msg.addString(database || '', this.options.encoding || DEFAULT_ENCODING);
    msg.addInt(1);  // Protocol version understood count.

    blr.addString(1, process.env['USER'] || process.env['USERNAME'] || 'Unknown', this.options.encoding || DEFAULT_ENCODING);
    var hostname = os.hostname();
    blr.addString(4, hostname, this.options.encoding || DEFAULT_ENCODING);
    blr.addBytes([6, 0]);
...
Connection.prototype.attach = function (options, callback, db) {
    ...
    blr.addByte(1);
    blr.addByteInt32(isc_dpb_sql_dialect, dialect); 
    blr.addString(isc_dpb_lc_ctype, this.options.encoding || 'UTF8', this.options.encoding || DEFAULT_ENCODING);
    blr.addString(isc_dpb_user_name, user, this.options.encoding || DEFAULT_ENCODING);
    blr.addString(isc_dpb_password, password, this.options.encoding || DEFAULT_ENCODING);

    if (role)
        blr.addString(isc_dpb_sql_role_name, role, this.options.encoding || DEFAULT_ENCODING);
...
Connection.prototype.createDatabase = function (options, callback) {
    ...
    blr.pos = 0;
    blr.addByte(1);
    blr.addString(isc_dpb_set_db_charset, this.options.encoding || 'UTF8', this.options.encoding || DEFAULT_ENCODING);
    blr.addString(isc_dpb_lc_ctype, this.options.encoding || 'UTF8', this.options.encoding || DEFAULT_ENCODING);
    blr.addString(isc_dpb_user_name, user, this.options.encoding || DEFAULT_ENCODING);
    blr.addString(isc_dpb_password, password, this.options.encoding || DEFAULT_ENCODING);
    ...
    while (br.pos < br.buffer.length) {
        switch (br.readByteCode()) {
            ...
            case isc_info_sql_get_plan:
                statement.plan = br.readString(this.options.encoding || DEFAULT_ENCODING);
                break;
                ...
                while (!finishDescribe){
                    switch (br.readByteCode()) {
                        ...
                        case isc_info_sql_field:
                            param.field = br.readString(this.options.encoding || DEFAULT_ENCODING);
                            break;
                        case isc_info_sql_relation:
                            param.relation = br.readString(this.options.encoding || DEFAULT_ENCODING);
                            break;
                        case isc_info_sql_owner:
                            param.owner = br.readString(this.options.encoding || DEFAULT_ENCODING);
                            break;
                        case isc_info_sql_alias:
                            param.alias = br.readString(this.options.encoding || DEFAULT_ENCODING);
                            break;
                        case isc_info_sql_relation_alias:
                            param.relationAlias = br.readString(this.options.encoding || DEFAULT_ENCODING);
                            break;
                        case isc_info_truncated:
...
Connection.prototype.prepareStatement = function (transaction, statement, query, plan, callback) {
    ...
    msg.addInt(transaction.handle);
    msg.addInt(statement.handle);
    msg.addInt(this.options.dialect || 3); // dialect = 3
    msg.addString(query, this.options.encoding || DEFAULT_ENCODING);
    msg.addBlr(blr);
        ...
        function putBlobData(index, value, callback) {
            ...
            self.createBlob2(transaction, function(err, blob) {
                ...
                if (Buffer.isBuffer(value))
                    ...
                    b = new Buffer(value, this.options.encoding || DEFAULT_ENCODING)
                else if (!isStream)
                    b = new Buffer(JSON.stringify(value), this.options.encoding || DEFAULT_ENCODING)
...
Connection.prototype.svcattach = function (options, callback, svc) {
    ...
    msg.addInt(0);
    msg.addString(DEFAULT_SVC_NAME, this.options.encoding || DEFAULT_ENCODING); // only local for moment
    msg.addBlr(this._blr);
